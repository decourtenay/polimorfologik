...
def get_input(phrase):
    ...
    print('Processing entry "' + str(entry_list[2:]) + '"')
    return entry_list
...
def find_pp(phrase):
    ...
    first_word = {'no.': '1', 'form': list_of_words[2], 'pos': ''}
    if len(list_of_words) > 3:
        second_word = {'no.': '2', 'form': list_of_words[3], 'pos': ''}
    else:
        second_word = {'no.': '2', 'form': '', 'pos': ''}
    if len(list_of_words) > 4:
        third_word = {'no.': '3', 'form': list_of_words[4], 'pos': ''}
    else:
        third_word = {'no.': '3', 'form': '', 'pos': ''}
    if len(list_of_words) > 5:
        fourth_word = {'no.': '4', 'form': list_of_words[5], 'pos': 'additional'}
    else:
        fourth_word = {'no.': '4', 'form': '', 'pos': ''}
    if len(list_of_words) > 6:
        fifth_word = {'no.': '5', 'form': list_of_words[6], 'pos': 'additional'}
    else:
        fifth_word = {'no.': '5', 'form': '', 'pos': ''}
    sixth_word = {'no.': '6', 'form': list_of_words[0], 'pos': 'additional'}
    seventh_word = {'no.': '6', 'form': list_of_words[1], 'pos': 'additional'}
    if list_of_words[2] in prepositions:
        first_word['pos'] = 'prep'
        if len(list_of_words) > 3:
            second_word['pos'] = 'prep_comp'
    elif len(list_of_words) > 3:
        if list_of_words[3] in prepositions:
            second_word['pos'] = 'prep'
            if len(list_of_words) > 4:
                third_word['pos'] = 'prep_comp'
    list_of_words_pos = [first_word, second_word, third_word, fourth_word, fifth_word, sixth_word, seventh_word]
...
def find_forms(phrase):
    ...
    for word in list_of_words_pos:
        ...
        if word['pos'] in ['prep', 'prep_comp', '', 'compl', 'additional']:
            ...
            plural_words.append(word_pl)
    for word in list_of_words_pos:
        if list_of_words_pos[6]['form'] == 'proper_noun':
            for case in ['nom', 'gen', 'dat', 'acc', 'inst', 'loc', 'voc']:
                if word[case] is not None:
                    if word['pos'] != 'prep':
                        word[case] = word[case].capitalize()
    for word in plural_words:
        if plural_words[6]['form'] == 'proper_noun':
            for case in ['nom', 'gen', 'dat', 'acc', 'inst', 'loc', 'voc']:
                if word[case] is not None:
                    if word['pos'] != 'prep':
                        word[case] = word[case].capitalize()
    ...
    for case in ['nom', 'gen', 'dat', 'acc', 'inst', 'loc', 'voc']:
        if second_pl[case] == '':
            ...
            final_forms_pl[case] = first_pl[case] + ' ' + second_pl[case] + ' ' + third_pl[case] + ' ' + fourth_pl[
                case] + ' ' + fifth_pl[case]
    final_forms_sg_str = mid['form'] + ';' + cat['form'] + ';' + final_forms_sg['nom'] + ';' + 'singular' + ';' \
                         + final_forms_sg['nom'] + ';' + final_forms_sg['gen'] + ';' + final_forms_sg['dat'] + ';' \
                         + final_forms_sg['acc'] + ';' + final_forms_sg['inst'] + ';' + final_forms_sg['loc'] + ';' \
                         + final_forms_sg['voc'] + '\n'
    final_forms_pl_str = mid['form'] + ';' + cat['form'] + ';' + final_forms_sg['nom'] + ';' + 'plural' + ';' \
